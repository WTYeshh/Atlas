...
class _DailyGreetingDialogState extends ConsumerState<DailyGreetingDialog> {
  ...
  List<String> getPersonalizedQuotes(String name) => [
    "Hey $name, let's seize the day! ✨",
    "Ready to conquer your goals, $name? 🚀",
    "Rise and shine, $name! You've got this. 💪",
    "Make today count, $name! 🌟",
    "Another day, another opportunity, $name! ☀️",
    "Hey $name, one step at a time! 🏃‍♂️",
    "Yo $name! Focus on the progress, not perfection. 📈",
    "Keep pushing forward, $name! 🏆",
    "Hey $name, start where you are, use what you have. 🎯",
    "Believing in you, $name! Let's do this. 🙌",
    "Hey $name, make today amazingly productive! ⚡",
    "Let's check off those tasks, $name! 📋",
  ];
  ...
  List<String> getNightQuotesAllCompleted(String name) => [
    "$name, you crushed all your goals today! 🏆",
    "Absolute legend, $name! 100% completed! 🌟",
    "Perfect score today, $name! Proud of you. 💪",
    "You did it, $name! Time for a well-deserved rest. 😴",
  ];

  List<String> getNightQuotesPending(String name) => [
    "Great effort today, $name! Tomorrow is a new start. 🌟",
    "Rest up, $name. Tomorrow we conquer the rest! 🔋",
    "You made solid progress today, $name! 👍",
    "Sleep well, $name. The remaining tasks can wait. 🛏️",
  ];

  List<String> getNightGreetingsAllCompleted(String name) => [
    "Incredible job, $name! Every single task checked off. Enjoy the cracker burst! 🎉",
    "A perfect checklist today, $name! Sleep tight, you've earned this peace. 🌌",
    "You completely cleared your plate, $name! Rest up and recharge. ⚡",
  ];

  List<String> getNightGreetingsPending(String name) => [
    "A few items are still pending. Use the reschedule option to push them to tomorrow! ➡️",
    "Almost done, $name! Reschedule the left-over tasks and get some rest. 🌙",
  ...
  void _updateModeTexts(
    ...
    final name = authState.userName ?? 'Friend';

    final personalizedQuotes = getPersonalizedQuotes(name);
    final nightQuotesAllCompleted = getNightQuotesAllCompleted(name);
    final nightQuotesPending = getNightQuotesPending(name);
    final nightGreetingsAllCompleted = getNightGreetingsAllCompleted(name);
    final nightGreetingsPending = getNightGreetingsPending(name);

    if (mode == GreetingMode.morning) {
      if (_dailyQuote.isEmpty || !personalizedQuotes.contains(_dailyQuote)) {
        _dailyQuote = personalizedQuotes[random.nextInt(personalizedQuotes.length)];
      }
    } else {
      if (taskProgress >= 0.999) {
        if (_dailyQuote.isEmpty || !nightQuotesAllCompleted.contains(_dailyQuote)) {
          _dailyQuote = nightQuotesAllCompleted[random.nextInt(nightQuotesAllCompleted.length)];
        }
      } else {
        if (_dailyQuote.isEmpty || !nightQuotesPending.contains(_dailyQuote)) {
          _dailyQuote = nightQuotesPending[random.nextInt(nightQuotesPending.length)];
        }
      }
    }
    ...
    if (mode == GreetingMode.morning) {
      ...
      final r = Random();
      if (taskProgress >= 0.999) {
        _greetingText = nightGreetingsAllCompleted[r.nextInt(nightGreetingsAllCompleted.length)];
      } else {
        _greetingText = nightGreetingsPending[r.nextInt(nightGreetingsPending.length)];
      }
    }
  }
